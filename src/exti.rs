...
pub fn configure_afio(port: u32, pin: u32)
{
    ...
    unsafe
    {
        let mut value = utils::read_register32(afio_exticr);
        value &= !(0x0F << ((pin % 4) * 4));
        value |= (port & 0x0F) << ((pin % 4) * 4); // Map the port to the pin
        utils::write_register32(afio_exticr, value);
    }
}
...
pub mod gpio
{
    ...
    pub fn set_edge(pin: u32, trigger: EdgeTrigger)
    {
        ...
        match trigger
        {
            EdgeTrigger::Falling =>
            {
                utils::clear_bit32(exti_ftsr,  pin as u8);
                utils::set_bit32(exti_ftsr,  pin as u8);
            },
            
            EdgeTrigger::Rising =>
            {
                utils::clear_bit32(exti_rtsr,  pin as u8);
                utils::set_bit32(exti_rtsr,  pin as u8);
            },
